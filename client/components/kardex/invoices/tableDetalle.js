...
const TableDetalle = () => {
  ...
  const { details, receiver, detailTotal } = useSelector(state => state.invoice);
  ...
  const handleSaveDetalle = async (id) => {
    ...
    return await detailRequest.storeMany(datos)
    .then(() => {
      Swal.fire({
        icon: 'success',
      ...
      <div className='table-responsive'>
        <Table striped>
          ...
          <tbody>
            {details?.length ? 
              details?.map((det) => 
                <Item key={`item-detalle-${det.objectId}`}
                  id={det.objectId}
        ...
        <Row>
          ...
          <Col md='4'>
            ...
            <div className='flex justify-between mb-3'>
              ...
              <b className='font-medium mt-2'>{format(detailTotal, { code: 'COP' })}</b>
            </div>
